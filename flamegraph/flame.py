...
if len(sys.argv) != 4:
    ...
    print "\tusage:", sys.argv[0], "<seconds> <identifer> <program_name>"
    sys.exit(1)

test_time = sys.argv[1]
identifer = sys.argv[2]
program = sys.argv[3]
pid = check_output(["pidof", program]).split()[0]
...
cmd2 = "grep " + program + " out.stacks | ./flamegraph.pl --width 1400 --color=io --title=\"File I/O Time Flame Graph\" --countname=us > io-flame-" + identifer + "-" + test_time +"secs" + ".svg"
cmd_io = [cmd1, cmd2]
io_th = threading.Thread(target = th_shell, args = (cmd_io, "<IO>  ", ))

'''
# drop page cache periodly
with open('/proc/sys/vm/drop_caches', 'w') as stream:
    stream.write("1\n");
